...
public final class TermuxService extends Service implements AppShell.AppShellClient, TermuxSession.TermuxSessionClient {
    ...
    public void onCreate() {
        Logger.logVerbose(LOG_TAG, "onCreate - Service starting");
        ...
        try {
            ...
            Logger.logVerbose(LOG_TAG, "Properties and ShellManager initialized, starting foreground");
            runStartForeground();

            SystemEventReceiver.registerPackageUpdateEvents(this);
            Logger.logVerbose(LOG_TAG, "onCreate completed successfully");
        } catch (Exception e) {
            Logger.logStackTraceWithMessage(LOG_TAG, "FATAL: TermuxService onCreate failed", e);
    ...
    private void runStartForeground() {
        ...
        try {
            ...
            // Handle ForegroundServiceStartNotAllowedException or SecurityException
            Logger.logStackTraceWithMessage(LOG_TAG, "Failed to start foreground service. This may happen if notification permission is denied or app is in background.", e);
            // Try to start without service type as fallback
            try {
                startForeground(TermuxConstants.TERMUX_APP_NOTIFICATION_ID, buildNotification());
            } catch (Exception e2) {
                Logger.logStackTraceWithMessage(LOG_TAG, "Failed to start foreground service even without type specification", e2);
            }
        }
    }
    ...
    private Notification buildNotification() {
        ...
        // Android 12+ requires FLAG_IMMUTABLE or FLAG_MUTABLE for PendingIntent
