...
public class TermuxSession {
    ...
    public static TermuxSession execute(@NonNull final Context currentPackageContext, @NonNull ExecutionCommand executionCommand,
        ...
        if (executionCommand.executable == null) {
            boolean useSafeShell = executionCommand.isFailsafe;
            
            if (!useSafeShell) {
                // Try to find Termux shell
                for (String shellBinary : UnixShellEnvironment.LOGIN_SHELL_BINARIES) {
                    ...
                    if (shellFile.exists() && shellFile.isFile()) {
                        ...
                        Logger.logVerbose(LOG_TAG, "Found login shell: " + shellFile.getAbsolutePath());
                        isLoginShell = true;
                        break;
                    }
                }
